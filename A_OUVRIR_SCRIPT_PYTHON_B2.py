...
def load_json(path):
    ...
    try:
        ...
        print(f"[WARNING] JSON invalide dans {path} : {error}")
        return None
...
def run_zfe():
    ...
    polygon = data.get("zfe_polygon", [])
    truck_path = data.get("truck_path", [])
...
def run_safety(threshold_g=2.5):
    events = []
    csv_path = Path("accelerometer_data.csv")

    if not csv_path.exists():
        print("[WARNING] fichier introuvable : accelerometer_data.csv")
        return events
    ...
    with open(csv_path, newline="", encoding="utf-8") as f:
        for line_number, row in enumerate(csv.DictReader(f), start=2):
            try:
                ...
                print(f"[WARNING] ligne CSV ignoree {line_number} : {error}")
                continue

            dynamic_g = math.sqrt(ax * ax + ay * ay)
    ...
    print(f"daily_score.json genere avec {len(events)} evenement(s) critique(s).")
    return events
...
if __name__ == "__main__":
    try:
        ...
        print(f"[ERROR CONTROLLED] execution arretee proprement : {error}")
